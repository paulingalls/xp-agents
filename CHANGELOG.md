...
## v2.30.8 — Sprint-acceptance criteria aligned with BDD doctrine + scaffold-acceptance M-3 ships

Two threads landed this session. The bigger thread shipped Milestone 3 of `/xp-scaffold-acceptance`: the on-disk write/install/verify pipeline with atomic revert, plus the SKILL.md wiring that drives it. The smaller thread fixed a doctrine-drift issue caught at retro — story acceptance criteria had been bullet-list prose for 12+ sprints despite `ACCEPTANCE_TESTING_DOCTRINE.md` requiring Given/When/Then since it shipped, because the `xp-sprint-start` template never reflected the doctrine.

- **`/xp-scaffold-acceptance` Milestone 3 (sprint-038).** New `scripts/scaffold_apply.py` exposes a phase-helper API (`create_snapshot` / `write_files` / `run_install` / `run_verify` / `revert`) plus an `apply_plan` driver that auto-reverts on any phase failure (write OSError, install/verify non-zero exit, timeout). Snapshot lives at `${TMPDIR}/scaffold-snap-<uuid12>/` with `backup/<relpath>` mirrors of `files_to_modify` targets and a persisted `plan.json` for cross-process re-load. `revert()` reads `files_to_create` from the in-memory snap so partial writes are unlinked even when `write_text_atomic` raises mid-pipeline. Subprocess discipline: `shell=False` + `shlex.split` + `stdout=DEVNULL` + `stderr=PIPE` + 300s install / 60s verify timeouts.
- **`apply_write_only` consolidates the validate→snapshot→write prefix.** Both `apply_plan` and the cycle-5 CLI `apply-write` subcommand drive it, eliminating duplication and ensuring the pre-existence guard runs identically through both entrypoints. CLI `_cmd_apply_write` shrank from ~30 lines to a 3-line wrapper.
- **`validate_plan` rejects pre-existing `files_to_create` targets.** xp-close-reviewer raised this as a Block on sprint-038: a misclassified plan (LLM treats an existing customer file as "create" instead of "modify") would silently overwrite + then unlink the file via `revert()` — silent data loss. Validation runs before any snapshot lands; `ApplyResult(ok=False, phase="write", reason=..., reverted=False, snapshot_id=None)` short-circuits with a reason naming every collision and pointing the caller at `files_to_modify`.
- **Revert-of-revert manual-recovery message.** When `revert()` itself fails (e.g., `shutil.copy2` raises `PermissionError` mid-restore), `_failure` populates `ApplyResult.recovery` with a string naming the snapshot directory and the unrestored relpaths so the customer has a non-zero path to clean up.
- **`scripts/scaffold_cli.py` apply-* subcommands (cycle 5).** New `apply-write`, `apply-install`, `apply-verify`, `apply-revert` subcommands consume plan JSON via stdin (apply-write) or `--snapshot-id` flag (others). `_run_apply_phase` consolidates install/verify (95% identical handlers). `apply-revert` is for explicit customer cancel only — phase failures already auto-revert internally so the skill cannot miss revert on a failure path. New `load_snapshot(snapshot_id, *, repo_root)` helper re-reads `snapshot_dir/plan.json` for cross-process state.
- **`ScaffoldPlan.files_to_create[].body` field** (resolves split-state contract retro Try). Bodies now live in the plan dict rather than the LLM's working memory; `render_preview(plan, show_files=True)` reads `body` from `$PLAN_JSON` and prints fenced sections under a `Files:` heading. Step 5's `show files` branch and Step 6's `apply-write` consume the same field — no more "drafted in working memory" implicit state.
- **`xp-scaffold-acceptance/SKILL.md` Steps 6-7 wired** with explicit `apply-write` → `apply-install` → `apply-verify` flow. Step 7 documents the conditional ("only run apply-verify if install came back ok=true") and pins `apply-revert` to the explicit-customer-cancel path. Step 6 documents the `files_to_modify` full-body deep-merge responsibility (`package.json` / `pyproject.toml` / `Cargo.toml`) — apply.py is format-agnostic so the skill must embed complete merged bodies. Runtime-order header added at top: `1 → 3 → 2 → 4 → 5 → 6 → 7` so readers don't misread doctrine numbering as execution order.
- **`xp-sprint-start/SKILL.md` Acceptance Criteria aligned with BDD doctrine.** Universal bullet (not just the optional capstone) now requires Given/When/Then prose with `And`/`But` continuations, the canonical `E2E:` prefix made explicit per "make implicit explicit," and a BDD-runner recommendation: when `system_context.json`'s `acceptance_surfaces` lists Cucumber/behave/SpecFlow, criteria are written as executable Gherkin `.feature` scenarios pointed at via `acceptance_execution.type`; otherwise prose GWT plus the project's native test runner is the default. Step 6 JSON example carries 3 criterion strings modeling basic, And-continuation, and E2E shapes plus a caption pointing BDD-runner readers at Gherkin substitution. No CLI validation added — codebase has no `stderr-warn + exit 0` precedent and the skill is the forcing function.
- **Plan-review concerns resolved on sprint-038.** Four medium concerns from xp-plan-reviewer all closed by sprint commits: `0e759f45fefd` (split-state body contract), `d790b92b8494` (inter-phase plan persistence), `cc703deb3980` (4-subcommand revert orchestration), `cbdb974b7fba` (manifest merge to caller).
