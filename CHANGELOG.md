...
## v4.20.0 — The SMM moves out of the deletable directory

**Minor: the SMM's default location changes.** The Claude plugins reference says
...
**Nothing moves in this release.** An SMM already under a plugin-data root is
DISCOVERED there and used in place. That leg is load-bearing, not politeness:
without it, upgrading would point every existing project at an empty new root,
which presents as a brand-new project with no history — exactly the loss being
fixed. Discovery checks a LIST of legacy roots (`$CLAUDE_PLUGIN_DATA` when set,
then the marketplace and dev-mode defaults), because that variable is absent in
some hook processes and a dev-mode install resolves the plugin id differently; a
single candidate would let a process that cannot see the real root mint an empty
