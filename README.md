...
**What it does:** Hooks fire automatically on every tool call. Before every write, a navigator checks your direction. After every write, a quality reviewer evaluates the code. Decisions, status, concerns, technical debt, and customer intent are broadcast to every agent through a shared event log. At session start, a retrospective shows what the team learned — including plugin health stats and undelivered customer requests. Tests must pass before the agent can stop.
...
- **Before every write** — SMM delta injected (two-tier: Active Context for quick checks, full SMM for writes), navigator provides strategic guidance
...
|---|---|---|
| `goal` | Project north star — what we're building and why | Customer proxy (first run) + customer |
| `customer_input` | The user's exact words | Hook (automatic) |
| `customer_intent` | Distilled customer request — tracked until delivered | Customer proxy (session start) |
...
| `debt` | Acknowledged tradeoff — ages and escalates across sessions | Quality reviewer + retrospective |
...
| `retrospective` | Keep/Fix/Try analysis + session stats | Hook (retrospective analyst) |
...
- **`strict`** (default) — TDD blocks on test failure, working_on conflicts block, navigator blocks on decision contradictions.
- **`advisory`** — same hooks fire, same events recorded, same retrospective runs — but nothing blocks. All blocks become warnings. The system still tracks everything; it just doesn't stop you.
