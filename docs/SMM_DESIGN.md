...
2. **Legacy discovery + relocation** — otherwise, and **only when `$XP_AGENTS_DATA` is unset**, look for an existing SMM under, in order, `$CLAUDE_PLUGIN_DATA` (when set), `~/.claude/plugins/data/xp-agents-xp-agents`, `~/.claude/plugins/data/xp-agents-inline`. First hit wins and is **copied** to the new root, which is then used. The source is never deleted, so any failure falls back to reading it. Relocation is skipped entirely while a teammate is live — worktree dirs under `{project-id}/worktrees/` or `.in-place-active-*` markers in the SMM — because a teammate's SMM path is pinned at spawn and cannot be redirected. A `.migrated-to` pointer at the old location redirects stragglers, and `init.sh` follows it even on the `$SMM_DIR`-pinned path.
