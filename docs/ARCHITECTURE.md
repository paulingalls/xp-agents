...
| **PostToolUse** | `Skill\|Agent` | `review_cycle_done.py` | Set review cycle flags — `simplify_done` from the `/code-review` skill, `quality_review_done` from the `xp-code-reviewer` agent's completion (NOT from `/xp-quality-review`: an inline skill's Skill call returns at launch, so its name would set the flag before the review ran); emit canonical lifecycle events (simplify/quality-review/security-review/plan-review/assign/housekeeping). Nudges next step via `additionalContext`; injects PROCESS_GUIDE.md after `xp-housekeeper` completes |
