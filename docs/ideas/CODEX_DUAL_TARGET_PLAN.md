...
1. **Install** xp-agents into Claude Code (`/plugin marketplace add paulingalls/xp-agents`)
   and into Codex (`codex plugin marketplace add paulingalls/xp-agents`).
...
copy, and a Claude lead relies on the user's one-time
`codex plugin marketplace add` for Codex teammates. No second checkout.
...
| 9 | SMM lives outside the worktree (`CLAUDE_PLUGIN_DATA`, user-level) — Codex `workspace-write` sandbox blocks the teammate's own `append.sh` shell calls. | High | Spawn with `danger-full-access` (parity with today's `--dangerously-skip-permissions`) or extend `sandbox_workspace_write.writable_roots` to include `$SMM_DIR` + `/tmp/xp-agents-teammates/`. Decide in P0. (P3) |
...
Setup: hand-write a minimal `.codex-plugin/plugin.json` + a codex `hooks.json`
containing only `SessionStart` (session_start.py), `PreToolUse:Bash`
(pre_tool_bash.py), `Stop` (teammate_stop_gate.py + tdd_stop_gate.py). Install
locally (`~/.codex/plugins/` or local marketplace path). Run one worktree
teammate via `codex exec --dangerously-bypass-hook-trust` with `SMM_DIR` +
`XP_TEAMMATE_NAME` exported.
...
- [ ] Sandbox decision (gap #9): confirm whether hook processes and teammate
      shell calls can write `$SMM_DIR` under `workspace-write` + writable_roots,
      or whether `danger-full-access` is required.
...
5. **Plugin-root self-resolution.** Scripts resolve plugin root relative to
   `__file__` first, env vars second — removes the assumption that the invoking
   harness exports `CLAUDE_PLUGIN_ROOT` into shell contexts.
...
1. `.codex-plugin/plugin.json` alongside `.claude-plugin/plugin.json` (same
   name/version/description; hooks field → `./hooks/hooks.codex.json`).
2. **Generated hooks variants.** One source spec (e.g.
   ...
   matchers, Claude-only events omitted). Checked in; a pin test asserts both
   are regeneration-clean so they cannot drift (fits the repo's pin-test
   culture).
3. Marketplace catalogs: `.claude-plugin/marketplace.json` (exists or add) and
   `.agents/plugins/marketplace.json` at repo root, both pointing at
   `./plugins/xp-agents`.
4. Install docs (README + PROCESS_GUIDE where user-facing): both one-liners,
   plus the Codex `/hooks` trust step (gap #10) and minimum version pins.
...
1. `spawn_teammate.py --provider codex`: build
   `codex exec - --json --ask-for-approval never --dangerously-bypass-hook-trust
   -o <last-msg-path>` + sandbox flags per the P0 decision (gap #9), `-m` /
   `-c model_reasoning_effort` from the provider-aware tier map (gap #12),
   cwd = worktree, prompt via stdin (preamble unchanged).
2. Preflight checks: codex binary on PATH, authenticated, plugin installed
   (else fail with the exact `codex plugin marketplace add` command to run).
...
1. Does `codex exec` load user-installed plugins in all cases, or only
   repo-trusted projects? (Affects preflight in P3.2 — may need trust
   pre-seeding in `~/.codex/config.toml` from the spawn script.)
