...
- **Constraints** — architectural/process bounds. Detail routes out: checkable claims→tests; history→git; comments→why code can't express; false claims→narrowed, not deleted. Cap 15-20.
...
**Plan cycle:** `/xp-schedule` → `EnterPlanMode` → `ExitPlanMode` → `/xp-review-plan` → `/xp-assign` → execute. Multi-file changes (3+ files). State-derived gates: the schedule gate blocks writes + plan-entry until `/xp-schedule` promotes; `.plan-awaiting-review` until reviewed; `.assign-pending` until assigned (teammate-mode plans only).

**Per commit (cadence set at kickoff):** *commit* — `/xp-quality-review` → `git commit`, gate blocks if skipped. *story* — gate defers; review at `/xp-story-close` Step 4.5b. At `/xp-{free,sprint,plan}-close`: threshold-gated `/code-review` (Step 4b, Skill tool) + LLM `/security-review` (Step 4). Deterministic patterns scan staged diffs.
...
**Multi-command AC:** `commands: list[str]` names the failing slot; single keeps `command`. Prefer `commands` over chained `&&` when mixing runners.
