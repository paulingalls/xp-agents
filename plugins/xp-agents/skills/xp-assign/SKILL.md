...
- **Read the task output file** named in the notification. `teammate_output_filter.py` does NOT tee its stdin, so this file stays ~empty during the run, then holds one of three shapes: **on success**, one summary line naming the **report file path** (`Report: <path> | Branch: <branch> | Cost: $<n>`); **on a run that reached a terminal event but died** (API drop mid-edit, max turns), a line opening `TEAMMATE FAILED (<reason>)` and a non-zero exit — the report file leads with the same header, the branch holds partial work, and this is a crash, not a delivery; **on a no-result failure**, several diagnostic lines plus a saved-artifact path — open that artifact for the full capture. Mid-flight, tail `$LOG_PATH` from Step 4 instead.
