...
- **File Domain**: Files this story claims while it runs. Two stories may declare the same file only when they can never run at the same time — one transitively depends on the other. Stories with no dependency between them need disjoint domains, or parallel teammates would step on each other; the sprint write refuses the collision. Building on an earlier story's file is normal: declare the dependency and share the path. A claim held by a finished, dropped, or never-started story never collides — re-touching its file needs no dependency edge. Always include test files alongside source files. For research stories with no expected code changes, use `[]` to mark the story code-free and prevent false pipeline-gap noise.
