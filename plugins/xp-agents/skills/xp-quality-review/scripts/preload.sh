...
# The reviewer receives the Constraints pillar (documented project conventions)
# via SubagentStart injection (_inject_full) and reads it directly in Section 1c
# to down-rate diffs that match a convention. No preload Design Context block is
# emitted — it was a second render built only for the removed risk classifier
# (sprint-113 story-002; the classifier had no SMM injection).
