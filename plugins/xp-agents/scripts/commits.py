...
def get_staged_diff(cwd: str) -> str:
    ...
    Returns empty string on git failure — caller treats absence-of-diff
    as no findings, leaving downstream gates to handle the underlying
    git issue.
    """
    out = _run_git(["git", "diff", "--cached"], cwd)
    return out or ""
