#!/usr/bin/env python3
"""PreToolUse:Skill hook — inject review guidance before skills run.

Courage means doing the hard thing: running all 3 review subagents during
/simplify, not skipping them because the change "looks small." This hook
reminds the agent before simplify runs, not after when it's too late.
"""
...
def run(input_data: dict, **_kwargs) -> str | None:
    """Inject courage nudge before /simplify runs."""
    if _common.is_xp_agent(input_data):
        return None

    skill = input_data.get("tool_input", {}).get("skill", "")
    if "simplify" not in skill:
        return None
