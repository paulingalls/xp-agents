...
def run(input_data: dict, smm_dir: Path | None = None) -> str | None:
    ...
    if smm_dir and not is_plan_file:
        is_smm_write = write_scope.is_smm_write(smm_dir, target_file, cwd)
        try:
        ...
        #
        # TWO PREDICATES, NEVER UNIONED. Both live in write_scope, and they
        ...
        # nothing else. Scope — "out of tree", "on a free branch", "mid-merge" —
        # says nothing about repairability, so folding the two into one
        # `is_exempt` would open the write door on a free branch while every
        # sprint gate is blind. Nor is one a subset of the other: $SMM_DIR is an
        # env var, so the SMM dir can sit INSIDE the working tree.
        # TestCorruptSprintKeepsThePredicatesApart goes red on the union.
        #
        # ORDER IS THE MECHANISM. `is_smm_write` is pure path math and stays
        # EAGER (the corrupt branch needs it first). `is_out_of_story_scope`
        # shells out to two uncached `git rev-parse` calls, so it stays LAZY and
        # LAST: Python short-circuits left to right, and the gate window is rare,
        # so the subprocesses are paid only when the gate is about to fire.
        # TestScheduleGateBranchProbeCost and TestScheduleGateMergeProbeCost both
        # go red if this term moves.
        if (
            sprint_data is not None
            and schedule_gate_active_data(sprint_data)
            and not is_smm_write
            and not write_scope.is_out_of_story_scope(target_file, cwd)
        ):
            raise _common.BlockedError(
