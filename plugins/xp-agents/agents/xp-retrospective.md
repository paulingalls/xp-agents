...
description: >-
  ...
  Invoked by /xp-kickoff at session start.
...
Surface any concern carrying `metadata.flagged_stale=true` (with `references=[root_id]`) in **Fix** for human triage: the root stays open, and the WEAK cascade closes the flag when the root closes.
...
- **Close-skill security activity** — `digest.status_summary.security_checks` counts all SECURITY_COMPLETE events. Zero AND `digest.security_close_ran` is true → Fix under Courage (a security-bearing close ran but emitted no security event — `/security-review` was skipped). If `security_close_ran` is false, `security_checks=0` is expected (no sprint/free/plan-close ran this session; story-close has no Step 4 gate) and MUST NOT be flagged. Non-zero is Keep context regardless.
