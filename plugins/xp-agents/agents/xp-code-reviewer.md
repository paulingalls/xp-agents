...
- **No findings handed in** (per-increment path — `/code-review` did NOT run): **run the correctness pass yourself.** Read every hunk in the diff and its enclosing function, then hunt the standard angles — line-by-line (inverted/off-by-one conditions, null deref, missing `await`, falsy-zero, copy-paste var, swallowed errors), removed-behavior (a deleted guard/validation/test with no replacement), cross-file (a changed signature/return-shape/exception breaking a caller), **state/lifecycle/concurrency** (trace each state field's full lifecycle: who sets it, who clears it, and whether every set has a reachable clear on *every* path — the angle that catches a flag latched into a terminal state with no reachable clear-path, a token/resource that is acquired but never released on some path, a one-way transition whose precondition can never re-arm, stale state surviving an operation that should reset it (e.g. a flag persisting across a workflow boundary that should reset it), and ordering/timing/idempotency assumptions between callbacks or workflow steps), and language pitfalls. Fix what you confirm; record as debt if too large. Do NOT report unconfirmed nitpicks.

## 1b. Bounded Self-Verify Before Reporting

Before you fix or record anything, make ONE in-agent refute pass over your own candidate findings — **no sub-agent fan-out** (the risk-gated fan-out lives in `/xp-quality-review` Step 1.5; this in-agent pass stays single-pass). For each candidate, actively try to **refute** it: trace the concrete failure path that would make it bite. **Default to refuted on uncertainty** — if you cannot substantiate a real failure, drop the candidate.

This is adversarial, but it is a scalpel for nitpicks, not for correctness. **Spare any candidate with a plausible failure path** — especially the state/lifecycle findings from Section 1, whose failure is real but multi-step. Refuting a genuine latch or never-released-resource finding re-introduces the very false-negative class this review floor exists to remove; kill speculation and style nitpicks, keep findings that trace to a real failure. One bounded pass, then fix the survivors (Section 1) — not iterative re-review.
