...
<!-- harness-ok: reasoning effort really is that harness's own setting; naming it generically would describe a knob that does not exist -->
...
<!-- harness-ok: same knob as above, named in the contract it is carried on -->
...
- **Positional-path runners** name their test file(s)/dir(s) as positional tokens on the command line: `pytest` / `python -m pytest`, `python -m unittest discover -s <path>` (also `-t <topdir>`), direct script invocations such as `python <path>` / `bash <path>`, `playwright test`, `jest`, `vitest`, `mocha`, `bun test <spec>`, `node --test`, `deno test`, `rspec`, `phpunit`, `mix test`, `dart`/`flutter test` — each optionally wrapped in `npx`/`bunx`/`pnpm exec`/`yarn`. The extracted paths are those positional tokens (with a `::selector` suffix stripped).
- **Whole-tree runners** name no file path on the command line; the proof file lives in `package.json`/config/scheme/module: npm/pnpm/yarn/lerna **script aliases** (`npm run test:e2e`, `pnpm test`) and bun's own alias forms (`bun run <script>`, `bun <script>:<suffix>`, e.g. `bun test:unit`), `turbo`, `nx`, `cargo`, `maven`, `gradle`, `xcodebuild`, `dotnet`, `ctest`, `go test`, `swift test`, `rake test`. These target the whole tree — any change matches. A script token like `test:e2e` is NOT a path.

A positional runner invoked with no path token (a bare `unittest discover`, or a whole-suite `playwright test`) also targets the whole tree. `bun test`'s positionals are filter **patterns**, not strict paths, so a bare-word filter (no `/` or `.`) is not path-shaped and correctly falls to the same whole-tree sentinel.
