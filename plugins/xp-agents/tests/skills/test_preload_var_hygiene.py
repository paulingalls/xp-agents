...
class TestEmitHelpers(_IntegrationTestCase):
    ...
    def test_emit_path_var_preserves_consecutive_spaces(self):
        """emit_path_var is the path-valued sibling of emit_var: it preserves a
        run of consecutive spaces (a worktree path may legitimately contain
        them — e.g. `/Users/John  Doe/proj`) while emit_var's `flat` collapses
        every whitespace run. The emitted TEAMMATE_CWD line feeds a downstream
        `git -C <path>` in the close/review skills, so a collapsed double space
        would target a non-existent directory."""
        r = self._run_helper('emit_path_var "$1" "$2"', "TEAMMATE_CWD", "/a/b  c/wt")
        self.assertEqual(r.returncode, 0, r.stderr)
        self.assertEqual(r.stdout, "TEAMMATE_CWD=/a/b  c/wt\n")

    def test_emit_path_var_neutralizes_forgery_vectors(self):
        """Spaces are preserved but the newline/CR/tab forgery vectors are not:
        each maps to a single space (strip_framing), so a crafted value emits
        exactly one line and cannot forge a second KEY= variable."""
        for value in ("a\nKEY2=evil", "a\rKEY2=evil", "a\tKEY2=evil"):
            with self.subTest(value=value):
                r = self._run_helper('emit_path_var "$1" "$2"', "TESTVAR", value)
                self.assertEqual(r.returncode, 0, r.stderr)
                self.assertEqual(r.stdout.count("\n"), 1, "exactly one line")
                self.assertNotIn("\nKEY2=", r.stdout, "no forged second variable")
                self.assertEqual(r.stdout, "TESTVAR=a KEY2=evil\n")

    def test_emit_path_var_empty_value(self):
        """An empty value emits `TESTVAR=` with a trailing newline only."""
        r = self._run_helper('emit_path_var "$1" "$2"', "TESTVAR", "")
        self.assertEqual(r.returncode, 0, r.stderr)
        self.assertEqual(r.stdout, "TESTVAR=\n")
