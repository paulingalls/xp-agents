#!/usr/bin/env python3
"""Milestone 8 capstone: the dual-packaging surfaces compose end to end.
...
read that reports the copy so installed.* That chain is what this file tests.
...
shipped manifest names a file that does not exist. There are three independent
copies of that string: in `manifest_emit.transform()`, in
`hooks_emit._VARIANT_NAME`, and in that test.

This file closes it by comparing two DERIVED values with no literal on either
side: the path the manifest declares, resolved against its own plugin root,
against the path the hooks emitter returns.
