...
_REGISTRY: dict[Site, Verdict] = {
    ...
    Site(
        ...
        "either names a plan file that no longer exists or holds no path at all, "
        "so no review could satisfy the gate and a lead left holding it is "
        "write-blocked with no way out. Both arms collect since story-023; the "
        "non-path one used to report and return, which left the armed state with "
        "no exit. Garbage collection of an unsatisfiable gate, not a discharge: "
        "the discharge lives at the plan reviewer's completion "
        ...
        "never got to satisfy — but no reviewable plan is named either way, so "
        "what is lost is the nudge to re-enter plan mode, not the review.\n"
